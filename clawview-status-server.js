...
function getAgentStatusV2(agentId) {
  ...
  // Format: {"timestamp": ISO, "state": "active"|"done"|"idle", "activity": "..."}
  ...
  if (fs.existsSync(statusFile)) {
    try {
      ...
      if (statusData.activity && statusAge < STATUS_FILE_ACTIVE_MAX_MS) {
        ...
        if (state === 'active') {
          agentReportedActive = true; // agent says it's working — trust it
        }
        // state=done/idle: use text but don't force active
      }
    } catch (e) {}
  }

  // Status determination:
  // 1. Agent self-reports active via .status → active (highest trust)
  // 2. Session updatedAt within 2min → active
  // 3. Otherwise → idle
  ...
  // Apply agent-reported activity text (parsed above before status determination)
  if (agentReportedActivity) {
  ...
  if ((wasRecentlyActive || agentReportedActive) && !activityText) {
    ...
    // Idle — neither session recency nor .status file says we're active
    // Only overwrite activityText if not already set by agent-reported
